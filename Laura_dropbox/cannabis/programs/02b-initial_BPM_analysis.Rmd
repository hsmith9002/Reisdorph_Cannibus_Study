---
title: "02b - Longitudinal Analysis of Cannabis-related heart rates (IP Administration)"
author: "Harry Smith"
date: "`r Sys.Date()`"
...
ip_model = lmer(bpm ~ time_window*dose*round + (1|mouse) , data=ip[ip$day_type=="dose",])
```
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-0lax">Variable describing aggregated time windows for bpm record collection. Original data had bpm data recorded <br>by the minute. These data were then averaged over 6 minute windows, and then aggregated in to hour long windows.<br>There 8 windows</td>
...
# difference in predose heart rate across time and dose
...
p <- ggplot(data = ip[ip$time_window=="PreDose",],
      xlab="Day of Study", ylab="heart rate During Predose Window")
p + geom_line(aes(x=study_day,y=bpm,col=dose,group=mouse))
```

Based on my understanding of the study design, the 2 dosage groups should not differ in heart rate at day 1 during the predose window because none of the animals would have received any cannabis prior to that time point. 

Since this graphic made it look like the dosage groups differed at day 1, I tested that directly.

```{r, echo=FALSE}
x <- lm(bpm ~ dose, data=ip[ip$study_day==1 & ip$time_window=="PreDose",])
...
```{r, echo=FALSE}
plot(as.numeric(ip$dose[ip$study_day==1 & ip$time_window=="PreDose"]), ip$bpm[ip$study_day==1 & ip$time_window=="PreDose"], xaxt="n", pch=19,
...
## Acute Effects of Cannabis on heart rate
...
### Initial Acute Effect of Cannabis on heart rate
...
```{r, echo=FALSE}
p <- emmip(x, dose ~ time_window, ylab = "heart rate in initial dose round",
      xlab = "")
p + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```


### Acute Effect of Cannabis on heart rate After Chronic Exposure to Cannabis
...
Based on my understanding of the study design, the 3 dosage groups should not differ in heart rate at day 1 during the predose window because none of the animals would have received any cannabis prior to that time point. 
...
There are significant differences in starting heart rate between the three dose levels.
