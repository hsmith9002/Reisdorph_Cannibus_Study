---
title: "02c - Longitudinal Analysis of Cannabis-related heart rates (Gavage Administration)"
author: "Harry Smith"
date: "`r Sys.Date()`"
...
gavage_model = lmer(bpm ~ time_window*dose*round + (1|mouse) , data=gavage[gavage$day_type=="dose",])
```
...
```{r, echo=FALSE}
x <- lm(bpm ~ dose, data=gavage[gavage$study_day==1 & gavage$time_window=="PreDose",])
...
```{r, echo=FALSE}
plot(as.numeric(gavage$dose[gavage$study_day==1 & gavage$time_window=="PreDose"]), gavage$bpm[gavage$study_day==1 & gavage$time_window=="PreDose"], xaxt="n", pch=19,
     xlab = "Dosage Group", ylab="heart rate")
axis(1, at=c(1:3), labels = levels(gavage$dose), tick=FALSE)
```    

There are no significant differences in starting heart rate between the three dose levels.
